...
# Swap to mp4
outputFile = pathlib.Path(inputFile).stem + ".mp4"

# output ffmpeg command
print("ffmpeg -y -i", "\"" + inputFile + "\"", vfilter, afilter, "-c:v libx264 -preset faster -profile high -crf 23", "-threads 13", "-c:a aac -b:a 192k", "\"video_output/" + outputFile + "\"")
